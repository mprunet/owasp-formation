...
public class OrderResource {
    ...
    private Order compute(List<PanierItem> items) {
        Order order = new Order();

        for (PanierItem pi : items) {
            pi.setQuantite(Math.max(pi.getQuantite(), 0));
            pi.setQuantite(Math.min(pi.getQuantite(), 10));
            pi.setProduct(products.get(pi.getProduct().getId()));
        }
    ...
    public ResponseEntity<Order> order(@Valid @RequestBody Order order) {
        Order o = compute(order.getItems());

        return ResponseEntity.ok()
            .headers(HeaderUtil.createAlert(applicationName, "Votre commande d'un montant de " + o.getTotal() + " Euros a été effectuée", ""))
            .body(order);
    }
